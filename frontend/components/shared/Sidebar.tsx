// Purpose: Main navigation sidebar — logo, company switcher, nav links, user menu
// Used by: app/dashboard/layout.tsx

"use client"

import Link from "next/link"
import { usePathname } from "next/navigation"
import {
  Bot,
  Building2,
  History,
  LayoutGrid,
  MessageSquare,
  Settings,
  User,
  Workflow,
} from "lucide-react"
import { Separator } from "@/components/ui/separator"
import { CompanySwitcher } from "./CompanySwitcher"
import { ClerkErrorBoundary } from "./ClerkErrorBoundary"
import { cn } from "@/lib/utils"

const NAV = [
  { href: "/dashboard/org-chart",  label: "Org Chart",  icon: Building2,     phase: 2 },
  { href: "/dashboard/library",    label: "Library",    icon: LayoutGrid,    phase: 2 },
  { href: "/dashboard/agents",     label: "Agents",     icon: Bot,           phase: 2 },
  { href: "/dashboard/pipelines",  label: "Pipelines",  icon: Workflow,      phase: 3 },
  { href: "/dashboard/chat",       label: "Chat",       icon: MessageSquare, phase: 4 },
  { href: "/dashboard/history",    label: "History",    icon: History,       phase: 5 },
]

const CURRENT_PHASE = 1

export function Sidebar() {
  const pathname = usePathname()

  return (
    <aside className="flex h-full w-56 shrink-0 flex-col border-r border-border bg-card">
      {/* Logo */}
      <div className="flex h-14 items-center px-4 border-b border-border">
        <span className="font-mono text-sm font-semibold text-foreground">korvuz.exe</span>
      </div>

      {/* Company switcher — safe: falls back to static display if Clerk is missing */}
      <div className="px-3 pt-3 pb-2">
        <ClerkErrorBoundary fallback={<StaticCompanyDisplay />}>
          <CompanySwitcher />
        </ClerkErrorBoundary>
      </div>

      <Separator className="mx-3" />

      {/* Nav */}
      <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-0.5">
        {NAV.map(({ href, label, icon: Icon, phase }) => {
          const active = pathname.startsWith(href)
          const locked = phase > CURRENT_PHASE
          return (
            <Link
              key={href}
              href={locked ? "#" : href}
              aria-disabled={locked}
              className={cn(
                "flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm transition-colors",
                locked
                  ? "cursor-not-allowed text-muted-foreground/40"
                  : active
                    ? "bg-primary/10 text-primary font-medium"
                    : "text-muted-foreground hover:bg-muted hover:text-foreground"
              )}
            >
              <Icon className="h-4 w-4 shrink-0" />
              {label}
              {locked && (
                <span className="ml-auto text-[10px] text-muted-foreground/40">soon</span>
              )}
            </Link>
          )
        })}
      </nav>

      <Separator className="mx-3" />

      {/* Settings + User */}
      <div className="px-2 py-3 space-y-0.5">
        <Link
          href="/dashboard/settings"
          className={cn(
            "flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm transition-colors",
            pathname.startsWith("/dashboard/settings")
              ? "bg-primary/10 text-primary font-medium"
              : "text-muted-foreground hover:bg-muted hover:text-foreground"
          )}
        >
          <Settings className="h-4 w-4 shrink-0" />
          Settings
        </Link>

        {/* User area — safe fallback if Clerk is not configured */}
        <ClerkErrorBoundary fallback={<StaticUserDisplay />}>
          <ClerkUserArea />
        </ClerkErrorBoundary>
      </div>
    </aside>
  )
}

/** Rendered inside ClerkErrorBoundary — safe to call Clerk hooks here */
function ClerkUserArea() {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { UserButton, useOrganization } = require("@clerk/nextjs")
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const { organization } = useOrganization()
  return (
    <div className="flex items-center gap-2.5 rounded-lg px-3 py-2">
      <UserButton afterSignOutUrl="/sign-in" />
      <span className="flex-1 truncate text-sm text-muted-foreground">
        {organization?.name ?? "Account"}
      </span>
    </div>
  )
}

function StaticCompanyDisplay() {
  return (
    <div className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-muted-foreground/60">
      <Building2 className="h-4 w-4" />
      <span className="flex-1 truncate">No company</span>
    </div>
  )
}

function StaticUserDisplay() {
  return (
    <div className="flex items-center gap-2.5 rounded-lg px-3 py-2">
      <div className="flex h-6 w-6 items-center justify-center rounded-full bg-muted">
        <User className="h-3.5 w-3.5 text-muted-foreground" />
      </div>
      <span className="flex-1 truncate text-sm text-muted-foreground">Account</span>
    </div>
  )
}
